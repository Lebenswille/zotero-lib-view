...
export class SettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(importLibrary)
			...
			.setDesc("Automatically update related notes when the BetterBibTex JSON file changes.")
			.addToggle((toggle) => toggle
				.setValue(settings.autoImportOnBibChange)
		...
		const saveLibraryColumns = async (columns: string[]) => {
		...
		const silentSaveLibraryColumns = async (columns: string[]) => {
			settings.libraryViewColumns = plugin.parseLibraryViewColumns(columns.join(","));
			await plugin.saveSettings();
			plugin.refreshLibraryViews();
		};
		...
		const renderLibraryColumnsEditor = () => {
			...
			new Setting(libraryColumnsContainer)
				.setName("Library View Columns")
				.setDesc("Customize each column: [Title] [Source Field] [Sub-Property]. Example: My Notes | notes | note");
			...
			currentColumns.forEach((columnString, index) => {
				...
				setting.addDropdown(dropdown => {
					...
					dropdown.onChange(async (value) => {
						parts[2] = value;
						// Correctly join parts back together
				...
				setting.addExtraButton(button => {
					button.setIcon("arrow-up")
						...
						.onClick(async () => {
							...
							await saveLibraryColumns(nextColumns);
						});
				}).addExtraButton(button => {
					button.setIcon("arrow-down")
						.setTooltip("Move down")
						.setDisabled(index === currentColumns.length - 1)
						.onClick(async () => {
					...
					button.setIcon("cross")
						...
						.setDisabled(currentColumns.length <= 1)
						.onClick(async () => {
			...
			new Setting(libraryColumnsContainer)
				.addButton(button => {
					button.setButtonText("Add Column")
						.onClick(async () => {
